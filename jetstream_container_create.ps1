...
$results = Invoke-RestMethod -Method Get -ContentType "application/json" -WebSession $session -URI "${BaseURL}/jetstream/datasource"
$status = ParseStatus $results "${ignore}"
...
$results = Invoke-RestMethod -URI "${BaseURL}/jetstream/container" -WebSession $session -Method Post -Body $json -ContentType 'application/json'
$status = ParseStatus $results "${ignore}"
