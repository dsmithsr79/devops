...
$results = Invoke-RestMethod -Method Get -ContentType "application/json" -WebSession $session -URI "${BaseURL}/source"
$status = ParseStatus $results "${ignore}"
...
if ("${ACTION}" -eq "status") {
   ...
   # Get Source Status ...
   #
   $results = Invoke-RestMethod -Method Get -ContentType "application/json" -WebSession $session -URI "${BaseURL}/source/${VDB}"
   $status = ParseStatus $results "${ignore}"
   #Write-Output "Source API Results: ${results}"
   ...
   if ( "${ACTION}" -eq "delete" ) {
      ...
      #
      $results = Invoke-RestMethod -URI "${BaseURL}/source/${VDB}/${ACTION}" -WebSession $session -Method Post -Body $json -ContentType 'application/json'
      $status = ParseStatus $results "${ignore}"
